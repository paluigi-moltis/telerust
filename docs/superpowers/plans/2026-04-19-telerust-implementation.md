...
**Tech Stack:** Rust, Tokio, Teloxide 0.17, Axum 0.8, Clap, Serde, TOML, Tracing, Anyhow, Directories 6, Keyring 3, Daemonize 0.5
...
- Create: `Cargo.toml`
- Create: `src/lib.rs`
- Create: `src/bin/telerust.rs`
- Modify: `.gitignore` (track `Cargo.lock` for binary crate)

- [ ] **Step 1: Initialize Cargo project**

```bash
cd /home/luigi/Documents/coding/telerust
cargo init --name telerust
```

This creates `Cargo.toml`, `src/main.rs`. We will restructure.

- [ ] **Step 2: Restructure into lib + bin layout**

Delete `src/main.rs`. Create `src/lib.rs` and `src/bin/telerust.rs`.
...
- [ ] **Step 4: Update .gitignore to track Cargo.lock**
...
mod tests {
    ...
    fn make_config(token: Option<&str>, use_keyring: bool) -> Config {
        Config {
            ...
            use_keyring,
            paired: PairedUser { username: None, user_id: None },
            ipc: IpcConfig::default(),
        }
    }
...
mod tests {
    ...
    fn test_inbound_message_serialization() {
        ...
        assert!(json.contains("\"message_id\":123"));
...
impl TelegramClient for TeloxideClient {
    async fn send_message(&self, chat_id: i64, request: &IpcRequest) -> Result<i64> {
        ...
        if let Some(reply_id) = request.reply_to_message_id {
            msg = msg.reply_to_message_id(teloxide::types::MessageId(reply_id as i32));
        }
    ...
    async fn start_polling(&self, handler: InboundHandler) -> Result<()> {
        ...
        let handler_fn = move |bot: teloxide::Bot, msg: Message| {
        ...
        Dispatcher::builder(
            self.bot.clone(),
            Update::filter_message().endpoint(handler_fn),
        )
        .build()
...
- Modify: `src/lib.rs` (if needed for re-exports)
...
pub async fn start(state: Arc<HttpServerState>, port: u16) -> anyhow::Result<()> {
    let app = router(state);
    let addr = std::net::SocketAddr::from(([127, 0, 0, 1], port));
    info!("HTTP IPC server listening on {addr}");
...
- [ ] **Step 2: Add Default derive for PairedUser in config.rs**

In `src/config.rs`, add `Default` to `PairedUser`:
```rust
...
- [ ] **Step 3: Make TeloxideClient accessible from cli module**

Add to `src/telegram.rs` a `pub use` or ensure `TeloxideClient` is public. It should already be `pub struct` — verify.

- [ ] **Step 4: Update bin/telerust.rs**
...
(daemon should already be there from Task 10.)
...
```bash
git add src/cli.rs src/bin/telerust.rs src/lib.rs src/config.rs
...
async fn test_http_send_reaches_telegram() {
    ...
    assert_eq!(sent[0].0, 42); // chat_id = paired user_id
...
- [ ] **Step 2: Add Clone derive to IpcRequest**

In `src/ipc/mod.rs`, ensure `IpcRequest` has `Clone`:
```rust
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IpcRequest {
```
...
- Initial implementation: project scaffold, config, token resolution, Telegram integration, HTTP and Unix IPC, daemon lifecycle, CLI commands, integration tests, CI
```
...
Plan complete and saved to `docs/superpowers/plans/2026-04-19-telerust-implementation.md`. Two execution options:
